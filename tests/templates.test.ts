...
test('Should render template conditional with empty strings', () => {
  ...
  expect(renderUktiTemplate(template, { qty: 1, isUnit: true })).toBe('There is 1 product available')
  expect(renderUktiTemplate(template, { qty: 3, isUnit: false })).toBe('There are 3 products available')
})
...
test('Should render template with multiple variables', () => {
  const template = 'The land vehicle{{length == 1 ? "" : "s"}} used {{length == 1 ? "is" : "are"}} {{items}} in the {{location}}.'
  const items = new Intl
    // @ts-expect-error browser api
    .ListFormat('en', { style: 'long', type: 'conjunction' })
    .format(['Motorcycle', 'Bus', 'Car'])

  expect(renderUktiTemplate(template, { items, length: items.length, location: 'countryside' }))
...
test('Should throw error if required variable is not defined and "throwIfError" enabled', () => {
  ...
  expect(() => renderUktiTemplate(template, {} as any, { throwIfError: true }))
    .toThrowError('Ukti template requires defined variable "qty" to render.')
})
